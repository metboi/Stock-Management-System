...
public class Funds {
    ...
    public void removeFunds(float amount, String username) {
        boolean checkLog = true;
        Scanner sc = new Scanner(System.in);
        PreparedStatement ps;
        int rs;
        ...
        while (checkLog){
            try {
                ...
                if(rs == 1)
                {
                    System.out.println("Money was removed from your account");
                    checkLog = false;
                }
                else{
                    System.err.println("Removing Funds Failed. Options: \n 1. Try again | 2. Abort");
                    int opt = sc.nextInt();
                    switch (opt){
                        case 1:
                            break;
